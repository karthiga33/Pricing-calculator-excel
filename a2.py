...
def landing_page():
    ...
    with col1:
        # Use on_change callback to navigate on single click
        st.button("Modernization Pathway", key="modernization_button", on_click=lambda: st.session_state.update(page='modernization'))
    
    with col2:
        st.button("Pricing Calculator", key="pricing_button", on_click=lambda: st.session_state.update(page='pricing'))
    
    with col3:
        st.button("Presales Assistants", key="presales_button", on_click=lambda: st.session_state.update(page='presales'))
    
    st.markdown("</div>", unsafe_allow_html=True)

# Presales Assistants Page
def presales_assistants():
    st.title("Presales Assistants")
    st.markdown("""
        Access the Presales Assistants tool here:  
        [Presales Assistants Link](https://95fjawvb.chat.qbusiness.us-west-2.on.aws/)
    """, unsafe_allow_html=True)
    
    # Back button to return to landing page
    st.button("Back to Home", key="presales_back_button", on_click=lambda: st.session_state.update(page='landing'))
...
def modernization_pathway():
    ...
    def determine_answer_widget(question: str):
        """Determine the type of answer widget for a question."""
        # Check for options in parentheses (e.g., "Select option: (A, B, C)")
        pattern = r"\(([^)]+)\)"
        ...
        # Define predefined dropdowns for common question patterns
        question_lower = question.lower()
        
        # Technology Stack related questions
        if any(keyword in question_lower for keyword in ["programming language", "language used", "tech stack", "technology stack"]):
            return {
                "type": "dropdown",
                "options": ["Java", "Python", ".NET", "Node.js", "PHP", "C++", "Go", "Ruby", "JavaScript", "C#", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["database", "db", "data storage"]):
            return {
                "type": "dropdown",
                "options": ["MySQL", "PostgreSQL", "Oracle", "SQL Server", "MongoDB", "DynamoDB", "Redis", "Cassandra", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["cloud provider", "current cloud", "hosting"]):
            return {
                "type": "dropdown",
                "options": ["AWS", "Azure", "Google Cloud", "On-premises", "Hybrid", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["environment", "deployment", "stage"]):
            return {
                "type": "dropdown",
                "options": ["Production", "Development", "Testing", "Staging", "Multiple", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["web server", "application server"]):
            return {
                "type": "dropdown",
                "options": ["Apache", "Nginx", "IIS", "Tomcat", "Node.js", "Express", "Other"]
            }
        
        # Security related questions
        if any(keyword in question_lower for keyword in ["authentication", "auth", "login"]):
            return {
                "type": "dropdown",
                "options": ["Active Directory", "LDAP", "OAuth", "SAML", "JWT", "Custom", "None", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["encryption", "encrypted"]):
            return {
                "type": "dropdown",
                "options": ["Yes - At Rest", "Yes - In Transit", "Yes - Both", "No", "Not Sure"]
            }
        
        if any(keyword in question_lower for keyword in ["firewall", "security measures"]):
            return {
                "type": "dropdown",
                "options": ["Hardware Firewall", "Software Firewall", "Cloud Firewall", "WAF", "Multiple", "None", "Other"]
            }
        
        # Code Quality related questions
        if any(keyword in question_lower for keyword in ["version control", "source control", "git"]):
            return {
                "type": "dropdown",
                "options": ["Git", "GitHub", "GitLab", "Bitbucket", "SVN", "TFS", "Mercurial", "None", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["ci/cd", "continuous integration", "deployment pipeline"]):
            return {
                "type": "dropdown",
                "options": ["Jenkins", "Azure DevOps", "GitHub Actions", "GitLab CI", "AWS CodePipeline", "CircleCI", "None", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["code review", "code quality"]):
            return {
                "type": "dropdown",
                "options": ["Manual Review", "Automated Tools", "SonarQube", "Code Climate", "Both", "None", "Other"]
            }
        
        # Testing related questions
        if any(keyword in question_lower for keyword in ["testing framework", "test automation", "testing tool"]):
            return {
                "type": "dropdown",
                "options": ["JUnit", "TestNG", "Selenium", "Cypress", "Jest", "PyTest", "Mocha", "None", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["testing type", "test coverage"]):
            return {
                "type": "dropdown",
                "options": ["Unit Testing", "Integration Testing", "E2E Testing", "Manual Testing", "All Types", "None", "Other"]
            }
        
        # AWS Infrastructure related questions
        if any(keyword in question_lower for keyword in ["instance type", "compute", "server size"]):
            return {
                "type": "dropdown",
                "options": ["t3.micro", "t3.small", "t3.medium", "m5.large", "c5.large", "r5.large", "Not Sure", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["region", "aws region", "location"]):
            return {
                "type": "dropdown",
                "options": ["us-east-1", "us-west-2", "eu-west-1", "ap-south-1", "ap-southeast-1", "Not Sure", "Other"]
            }
        
        if any(keyword in question_lower for keyword in ["monitoring", "logging"]):
            return {
                "type": "dropdown",
                "options": ["CloudWatch", "Splunk", "ELK Stack", "Datadog", "New Relic", "Custom", "None", "Other"]
            }
        
        # Yes/No questions
        if any(keyword in question_lower for keyword in ["do you have", "are you using", "is there", "do you use"]):
            return {
                "type": "dropdown",
                "options": ["Yes", "No", "Not Sure"]
            }
        
        # Scale/Size related questions
        if any(keyword in question_lower for keyword in ["size", "scale", "users", "concurrent"]):
            return {
                "type": "dropdown",
                "options": ["Small (< 100 users)", "Medium (100-1000 users)", "Large (1000-10000 users)", "Enterprise (> 10000 users)", "Not Sure"]
            }
        
        # Performance related questions
        if any(keyword in question_lower for keyword in ["performance", "load", "response time"]):
            return {
                "type": "dropdown",
                "options": ["Excellent", "Good", "Average", "Poor", "Not Measured", "Other"]
            }
    ...
    def main_modernization():
        ...
        for module, questions in modules_with_questions.items():
            ...
            for question in questions:
                widget_info = determine_answer_widget(question)
                
                if widget_info["type"] == "dropdown":
                    # Show dropdown with options
                    response = st.selectbox(
                        f"{question}", 
                        options=["Select an option..."] + widget_info["options"], 
                        key=question
                    )
                    # If "Other" is selected, provide additional text input
                    if response == "Other":
                        other_response = st.text_input(
                            f"Please specify for: {question}", 
                            key=f"{question}_other"
                        )
                        user_responses[question] = other_response if other_response else "Other (not specified)"
                    else:
                        user_responses[question] = response if response != "Select an option..." else "No response provided"
                else:
                    # Show manual text area for questions without dropdown options
                    response = st.text_area(f"{question}", key=question)
                    user_responses[question] = response if response else "No response provided"
        
        all_filled = all(value.strip() != "" and value != "No response provided" and value != "Select an option..." for value in user_responses.values())
        if all_filled:
        ...
        # Back button to return to landing page
        st.button("Back to Home", key="modernization_back_button", on_click=lambda: st.session_state.update(page='landing'))
...
def pricing_calculator():
    ...
    def main_pricing():
        ...
        # Back button to return to landing page
        st.button("Back to Home", key="pricing_back_button", on_click=lambda: st.session_state.update(page='landing'))
...
def main():
    if st.session_state.page == 'landing':
        ...
        pricing_calculator()
    elif st.session_state.page == 'presales':
        presales_assistants()
